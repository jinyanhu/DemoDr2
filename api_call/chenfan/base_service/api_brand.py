...
class ApiBrand(BaseHttp):
    ...
    def get_list(self, body_data=None):
        ...
        response = self.http.get(url, params=body_data)       # post方法

        # 返回响应数据
        return response

    def switch_brand(self, body_data=None):
        """
        禁用启用品牌
        """
        # 用字典字段赋值的方式，对header某个需要变更的字段赋值
        ...
        # 调用对应http方法，加入body参数，发送请求
        url = self.url + "/chenfan_base/brand/switchBanBrand"
        response = self.http.put(url, body=body_data)       # post方法
